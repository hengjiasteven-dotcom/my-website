import * as THREE from '../js/vendor/three/build/three.module.js';
    ...
    const WORLD_ASSET_BASE = (function() {
      var configured = (window.DREAM_WORLD_ASSET_BASE || '').trim().replace(/\/+$/, '');
      return configured || '';
    })();
    const PUBLIC_API_BASE = (function() {
      var configured = (window.DREAM_PUBLIC_API_BASE || '').trim().replace(/\/+$/, '');
    ...
    function worldApiUrl(apiPath) {
      var normalized = String(apiPath || '');
      if (!normalized) return PUBLIC_API_BASE || '';
    ...
    const WORLD_CHAT_API_URL = worldApiUrl('/api/world-chat');
