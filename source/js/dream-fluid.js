(function() {
  ...
  function apiUrl(path) {
    ...
    if (!normalizedPath) return publicApiBase || '';
    if (/^https?:\/\//i.test(normalizedPath)) return normalizedPath;
    if (normalizedPath.charAt(0) !== '/') normalizedPath = '/' + normalizedPath;
    return publicApiBase ? publicApiBase + normalizedPath : normalizedPath;
  }
  ...
  function setSingleCounterText(selector, text) {
    ...
    return fetch(url, options).then(function(response) {
      return response.json().then(function(payload) {
        if (!response.ok || !payload || payload.ok === false) {
          throw new Error(payload && payload.error ? payload.error : '访问统计请求失败');
        }
        return payload.data || {};
      });
    });
  ...
  function initVisitCounters() {
    ...
    appendPostPageview(currentPath);
    setSingleCounterText('[data-dream-article-view]', '加载中');

    runWhenIdle(function() {
      requestJson(apiUrl('/api/v1/visits/hit'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pagePath: currentPath })
      })
      .then(function(data) {
        setSingleCounterText('[data-dream-visit-total]', formatNumber(data.totalVisits));
        setSingleCounterText('[data-dream-visit-today]', formatNumber(data.todayVisits));
        setSingleCounterText('[data-dream-visitor-total]', formatNumber(data.totalVisitors));
        setSingleCounterText('[data-dream-visitor-today]', formatNumber(data.todayVisitors));
        setSingleCounterText('[data-dream-article-view]', formatNumber(data.pageVisits));
      })
      .catch(function(error) {
        setCounterText(counterLoadingSelector, '暂不可用');
        setSingleCounterText('[data-dream-article-view]', '暂不可用');
  ...
  function buildVideoSpotlightMarkup(kind) {
    ...
    return [
      ...
      '</div>',
      '</div>',
      isHome ? '<a class="dream-blog-publish-btn" href="https://api.xiaodaidai.site/admin" target="_blank" rel="noopener" data-no-pjax title="打开博客后台">我要发博客</a>' : '',
  ...
  function createNetworkPet() {
    ...
    function petChatEndpoints() {
      var primary = apiUrl('/api/pet-chat');
...
(function(){
  var observer = new MutationObserver(function(){
    ...
    if(!audio) return;
    audio.addEventListener('play', function(){
      localStorage.setItem('blog_music_playing', '1');
      localStorage.setItem('blog_music_start', String(Date.now()));
    });
    audio.addEventListener('pause', function(){
      localStorage.setItem('blog_music_playing', '0');
      var prev = Number(localStorage.getItem('blog_music_elapsed')||0);
      var start = Number(localStorage.getItem('blog_music_start')||0);
      if(start>0) localStorage.setItem('blog_music_elapsed', String(prev+(Date.now()-start)));
      localStorage.setItem('blog_music_start', '0');
    });
    audio.addEventListener('ended', function(){
      localStorage.setItem('blog_music_playing', '0');
      var prev = Number(localStorage.getItem('blog_music_elapsed')||0);
      ...
      if(start>0) localStorage.setItem('blog_music_elapsed', String(prev+(Date.now()-start)));
      localStorage.setItem('blog_music_start', '0');
      localStorage.setItem('blog_music_song_done', String(Date.now()));
    });
    // check initial state
    if(!audio.paused){
      localStorage.setItem('blog_music_playing', '1');
...
(function(){
  function countArticleWords(){
    var total = 0;
    // 只在文章详情页统计（URL 含日期格式或 post 路径）
    var path = window.location.pathname;
    var isPostPage = /\d{4}\/\d{2}\/\d{2}/.test(path) || /\/post\//.test(path) || document.querySelector(".markdown-body");
    if(!isPostPage) return;
    // 只统计文章正文 .markdown-body
    var body = document.querySelector(".markdown-body");
    if(!body) return;
    // 排除代码块和 pre 标签
    var clone = body.cloneNode(true);
    var codes = clone.querySelectorAll("pre,code,script,style");
    for(var i=0;i<codes.length;i++) codes[i].remove();
    var text = (clone.textContent || "").replace(/\s/g,"");
    total = text.length;
    if(total>0){
      localStorage.setItem("blog_word_count", String(total));
      localStorage.setItem("blog_word_count_time", String(Date.now()));
    }
  }// recount when pjax navigation completes
  document.addEventListener('pjax:complete', countArticleWords);
  // also recount when page fully loads
  window.addEventListener('load', function(){ setTimeout(countArticleWords, 1000); });
})();
